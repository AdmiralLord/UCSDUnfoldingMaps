...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		fill(255, 0, 0);
		triangle(42,275,50,291,58,275);
		
		fill(0, 0, 0);
		text("In Land", 75, 125);
		text("In Ocean", 75, 175);
	...
	private boolean isLand(PointFeature earthquake) {
		...
		for (Marker m : countryMarkers) {
			...
			//dodalem ifa
			//System.out.println(isInCountry(earthquake,m));
	...
	private void printQuakes() 
	{
		...
		//}
			for(Marker qm : quakeMarkers) {
				if((qm.getClass()).getName() == "module4.LandQuakeMarker"){
					//System.out.println(((LandQuakeMarker)qm).getCountry());
					name2=((LandQuakeMarker)qm).getCountry();
					
					if(!(quakeMap.containsKey(name2))) {
						quakeMap.put(name2,1);
					}
					else {
						quakeMap.put(name2,(quakeMap.get(name2)+1));
					}
				}
				else{
					oceanQuakes++;
			...
			//System.out.println(qm.getClass());	
	
			for (String s : quakeMap.keySet()) {
				System.out.println(s + ": " + quakeMap.get(s));
			}
